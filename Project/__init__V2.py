...
if __name__ == '__main__':
    ...
    index = 0
    for dpth in range(5, 6):
        print(dpth)
        for min_smpl in range(5, 6):
            print(min_smpl)
            start = time.time()
            RF = RandomForest(n_trees=10, n_features=n_features, max_depth=dpth, min_samples_leaf=min_smpl)
    ...
    results_df.to_csv("UNSW_MYRF10.csv", index=False, header=True)
    '''
