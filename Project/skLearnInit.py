...
if __name__ == '__main__':
    data = pd.read_csv('SAHeart.data', sep=',', header=0, index_col=0)
    ...
    for dpth in range(2, 10):
        for min_smpl in range(2, 10):
            start = time.time()
            RF = RandomForestClassifier(n_estimators=100, max_depth=dpth, random_state=0, max_features=3,
    ...
    results_df.to_csv("SAHeart_RF100.csv", index=False, header=True)
